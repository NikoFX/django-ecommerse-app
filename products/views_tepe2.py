...
def products(request):
    ...
    return render(request, 'products/tepe/templates/products/products.html', {'pr': pr, 'pg': pg, 'pageminus': pageminus, 'pageplus': pageplus, 'page_number': page_number})

def interdental(request):
    pr = Products.objects.filter(Q(status=True) & Q(Q(company='all') | Q(company='tepe')) & (Q(category='original') | Q(category='extra_soft') | Q(category='angle')))
    ...
    return render(request, 'products/tepe/templates/products/products.html', {'pr': pr, 'pg': pg, 'pageminus': pageminus, 'pageplus': pageplus})
...
def brushers(request):
    pr = Products.objects.filter(Q(status=True) & Q(Q(company='all') | Q(company='tepe')) & Q(Q(category='adult') | Q(category='kids')))
...
def accesories(request):
    pr = Products.objects.filter(Q(status=True) & Q(Q(company='all') | Q(company='tepe')) & Q(category='accesories'))
...
def add(request):
    ...
    try:
        ...
        # create #
        b = Basket(cookie_id=cookie_id, product_id=product_id, name=get_pr.name, size=get_pr.size, image=get_pr.image1, price=get_pr.price, count=count)
        b.save()
...
def product_detail(request, product_id=0):
    ...
    return render(request, 'products/tepe/templates/products/product_info.html', {'pr': pr})